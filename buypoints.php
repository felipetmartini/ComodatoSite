...
<table id="buypointsTable" class="table table-striped table-hover">
	<tr class="yellow">
		<th>Price:</th>
		<th>Points:</th>
		<?php if ($pagseguro['showBonus']) { ?>
			<th>Bonus:</th>
		<?php } ?>
		<th>Action:</th>
	</tr>
		<?php
		foreach ($psprices as $psprice => $pspoints) {
		echo '<tr class="special">';
		echo '<td>'. $psprice .'('. $paypal['currency'] .')</td>';
		echo '<td>'. $pspoints .'</td>';
		if ($pagseguro['showBonus']) echo '<td>'. calculate_discount(($pagseguro['points_per_currency'] * $psprice), $pspoints) .' bonus</td>';
		?>
		<td>
			...
			<form target="pagseguro" method="post" action="https://pagseguro.uol.com.br/v2/checkout/payment.html">
				...
				<input type="hidden" name="itemDescription1" value="<?php echo $pspoints .' shop points on '. $config['site_title'] .' in account '. (int)$_SESSION['user_id'];?>">
