<?php require_once 'engine/init.php'; include 'layout/overall/header.php';
    ...
    if ($cache->hasExpired()) {
        $addons = mysql_select_multi("SELECT `p`.`name` AS `name`, `p`.`level` AS `level`, `p`.`group_id` AS `group_id`, SUM(`ps`.`value` & 255) AS `sum` FROM `player_storage` ps LEFT JOIN `players` p ON `ps`.`player_id` = `p`.`id` WHERE `ps`.`key` BETWEEN 10001501 AND 10001525 AND level >= 8 AND group_id < 4 GROUP BY `name` ORDER BY `sum` DESC, `level` DESC LIMIT 50;");
